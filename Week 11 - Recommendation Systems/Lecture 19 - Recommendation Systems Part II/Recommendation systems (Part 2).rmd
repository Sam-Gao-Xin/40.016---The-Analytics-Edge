...
The data come from [grouplens](https://grouplens.org), a research lab in the Department of Computer Science and Engineering at the University of Minnesota. We begin by loading the data. 
...
for(i in 1:length(spl1c)){
  for(j in 1:length(spl1)){
      #One row of correlation values of one splc and every spl1
      Cor[j] <- cor(Data[spl1c[i],spl2],Data[spl1[j],spl2],use = "pairwise.complete.obs") 
  }
  #Order that row of correlation values
  V <- order(Cor, decreasing=TRUE, na.last=NA)
  #Add it to the Order Matrix
...
Now, we compute user predictions by looking at the 250 nearest neighbors and averaging equally over all these user ratings in the items in `spl2c`:
